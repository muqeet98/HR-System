...
class LeaveReqDetail extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      leave_type: '',
      date_from: Moment('').format('Do  MMM  YYYY') ,
      date_to: Moment('').format('Do  MMM  YYYY') ,
      leave_id: '',
      leave_description: '',
      leave_status: '',
      emp_id: '',
...
RejectLeave =() => {
  ...
  fetch('http://' + global.IP + '/BIIT_HR/api/leaves/RejectLeave', {
    method: 'POST',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json'
  ...
  // this.setState({leave_type : leave_type})
    Moment.locale('en');
    var dt = this.state.date_from;
    var st= this.state.date_to;
    return (
      <ScrollView style={styles.container}>
        ...
        <KeyboardAvoidingView behavior="height" style={styles.container}>
          <TouchableWithoutFeedback style={styles.container} onPress={Keyboard.dismiss}>
              ...
              <View style={{ flexDirection: 'row' }}>
                <View >
                  <Image source={{ uri: 'data:image/jpeg;base64,' + dp }} style={{
                    width: 100,
                    height: 100,
                    borderRadius: 20, marginLeft: ('15%'), marginTop: ('12%')
                  }} />
                </View>
                <View>
                  <View style={{ flexDirection: 'row', marginTop: ('20%'), fontSize: 16 }}>
                    {/* <Text style={styles.textstyle}>Name: </Text> */}
                    <Text style={{ fontSize: 18, marginLeft: ('5%'), fontWeight: 'bold' }}>{name}</Text>
                  </View>

                  <View style={{ flexDirection: 'row', marginTop: 10, fontSize: 20 }}>

                    <Text style={{ fontSize: 16, marginLeft: ('15%') }}>{desig}</Text>
                  </View>
            ...
            <TouchableHighlight style={{borderWidth:2 , borderColor: 'green', borderRadius: 10, width: 120, height: 25}} onPress = {() => {this.toggleModal(true)}}>
...
<Text style={{ fontSize: 20, marginLeft: ('20%') }}>{desig}</Text>
            ...
            <View style={{marginLeft: 10}}>
              <View style={{ flexDirection: 'row', marginTop: 30, fontSize: 20 }}>
                ...
                {/* <TouchableOpacity onPress={() => alert(this.state.date_from)}>
                  <Text>Click Me</Text>
                </TouchableOpacity> */}
              </View>
...
<Text style={{ fontSize: 18, marginLeft: ('20%') }}>{Moment(dt).format("Do  MMM  YYYY")}</Text>
              ...
              <View style={{ marginTop: 30, flexDirection: 'row' }}>
                ...
                <Text style={{ fontSize: 18, marginLeft: ('20%') }}>     {Moment(st).format("Do  MMM  YYYY")}</Text>
              </View>
              </View>
              ...
              <View style={styles.buttonContainer}>
                <TouchableOpacity style={styles.buttons1} onPress={this.RejectLeave}>
